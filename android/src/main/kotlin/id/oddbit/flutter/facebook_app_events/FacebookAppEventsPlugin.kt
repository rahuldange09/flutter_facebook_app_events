...
class FacebookAppEventsPlugin(registrar: Registrar) : MethodCallHandler {
  ...
  companion object {
    ...
    private fun logPurchase(call: MethodCall, result: Result) {
        val parameters = call.argument("parameters") as? Map<String, Any>
        val valueToSum: Double? = call.argument("_valueToSum")
        val bigDecimal = BigDecimal.valueOf(valueToSum!!)
        val currency = Currency.getInstance(call.argument("fb_currency") as? String)

        if (bigDecimal != null && parameters != null && currency != null) {
            val parameterBundle = createBundleFromMap(parameters)
            appEventsLogger.logPurchase(bigDecimal, currency, parameterBundle)
          result.success(true)
        }else {
